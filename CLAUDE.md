...
- **Subtitles** — `subtitle/` is a provider adapter, not one vendor. `provider.go` owns the `Provider` interface (`Name`/`Search`/`DownloadVTT`), `Creds`, the per-provider `CredsResolver`, and the opaque-id helpers; `subdl.go` is the one implementation wired in; `vtt.go`/`archive.go`/`httpx.go` hold the shared SRT→VTT, zip/gzip and fetch helpers. `service.go` resolves creds and dispatches.
  - `opensubtitles.go` is **kept but not wired in**: `Module` registers SubDL alone, no credentials reach it, and an `opensubtitles:` id now returns 400 "unknown provider". The file compiles so it can't rot; its header comment lists the three steps to wire it back in.
