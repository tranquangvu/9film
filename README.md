...
1. Open the `.dmg` and drag **9film** into Applications.
   ...
   <img src="docs/assets/install-drag.svg" alt="The mounted disk image: the 9film app on the left, an Applications shortcut on the right" width="560">
...
2. The first launch is refused — the app is ad-hoc signed, not notarized. Open **System Settings → Privacy & Security**, scroll to **Security**, and click **Open Anyway**. Once, and never again.
   ...
   <img src="docs/assets/open-anyway.svg" alt="System Settings, Privacy &amp; Security: the row reading “9film.app” was blocked to protect your Mac, with the Open Anyway button beside it" width="700">
...
Vite proxies `/api` and `/hls` to the backend, and `./9film.db` is created, migrated and seeded on first run. Every other command, and the three environment variables the backend reads, are in [docs/getting-started.md](docs/getting-started.md).
...
<img src="docs/assets/architecture.svg" alt="The browser and the desktop window both talk to the Go backend, which is the only thing that reaches IMDb, the stream CDN, SubDL and Gemini, and the only thing that touches the SQLite file" width="820">
...
Each backend module is a vertical slice: `repo.go` → `service.go` → `handler.go` → `route.go`, wired by `module.go`. [docs/architecture.md](docs/architecture.md) walks through it; `CLAUDE.md` carries the constraints for whoever edits the code.

## Docs

[Getting started](docs/getting-started.md) · [Architecture](docs/architecture.md) · [API](docs/api.md)
