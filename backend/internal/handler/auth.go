...
func issueAndRespond(c *gin.Context, cfg *config.Config, u *store.User, status int) {
	token, err := auth.Issue(u.ID, cfg.JWTSecret, cfg.TokenTTL)
	if err != nil {
...
func Signup(st *store.Store, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		u, err := st.CreateUser(email, hash, name, avatarFor(email))
		if err != nil {
			logger.Get().Warn("create user failed", zap.Error(err))
			c.JSON(http.StatusInternalServerError, gin.H{"error": "could not create account"})
			return
		}
		issueAndRespond(c, cfg, u, http.StatusCreated)
	}
}

// Login verifies credentials and returns a token.
func Login(st *store.Store, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		issueAndRespond(c, cfg, u, http.StatusOK)
	}
}
