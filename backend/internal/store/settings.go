...
func defaultSettings() Settings {
	return Settings{AutoplayNext: true, DefaultSubtitleLang: "en", DefaultQuality: "auto"}
}
...
func (s *Store) GetSettings(userID int64) (Settings, error) {
	...
	var autoplay int
	err := s.db.QueryRow(
		`SELECT autoplay_next, default_subtitle_lang, default_quality FROM settings WHERE user_id = ?`,
		userID,
	).Scan(&autoplay, &st.DefaultSubtitleLang, &st.DefaultQuality)
...
func (s *Store) UpsertSettings(userID int64, st Settings) error {
	...
	_, err := s.db.Exec(
		`INSERT INTO settings (user_id, autoplay_next, default_subtitle_lang, default_quality, updated_at)
		   VALUES (?, ?, ?, ?, datetime('now'))
		...
		userID, autoplay, st.DefaultSubtitleLang, st.DefaultQuality,
	)
	return err
}
