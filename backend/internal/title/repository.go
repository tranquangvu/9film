...
// Repository serves the IMDb GraphQL integration. It owns the HTTP client and
// the short-lived per-title detail cache, so a single shared instance keeps the
// cache coherent across all callers. Methods return the raw, unflattened
// ImdbTitle shape; the Service flattens into the Title DTO.
