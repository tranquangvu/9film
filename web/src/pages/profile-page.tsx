...
function ProfileCard() {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      await updateMe.mutateAsync({ avatar });
      setEditing(false);
      ...
      <div className="space-y-5">
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-1 text-lg font-bold text-white truncate">@{user?.username}</p>
            <p className="text-xs text-zinc-500 mt-0.5">
              Fixed — it's how the app finds your local account.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
            {avatarOptions(user?.username ?? '').map((url) => (
              <button
                key={url}
