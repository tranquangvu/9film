...
export default function TvSeriesPage() {
  ...
  const filtered = useMemo(() => {
    let result = toMovies(browse.data?.titles ?? []).filter((m) => m.type === 'series');
    ...
    <div className="min-h-screen bg-background pb-16">
      <div className="pt-24 pb-6 px-4 md:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="text-zinc-500 mt-1 text-sm">Binge-worthy shows from around the world</p>
        </motion.div>
      </div>
      ...
      <div className="px-4 md:px-8 lg:px-12 mt-6">
        <AnimatePresence mode="wait">
          {browse.isLoading ? (
            <div className="text-zinc-500 text-sm py-16 text-center">Loading series…</div>
          ) : filtered.length === 0 ? (
