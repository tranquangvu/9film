...
export async function getTitle(imdbId: string, signal?: AbortSignal): Promise<TitleDetail> {
  ...
  try {
    const json = await apiFetch<TitleDetail>(`/api/title/${id}`, { signal });
...
export async function searchTitles(q: string, limit = 20, signal?: AbortSignal): Promise<TitleDetail[]> {
  const params = new URLSearchParams({ q, limit: String(limit) });
  const json = await apiFetch<{ titles?: TitleDetail[] }>(`/api/title/search?${params}`, { signal });
  return json.titles ?? [];
}

export async function getTrendingTitles(limit = 10, signal?: AbortSignal): Promise<TitleDetail[]> {
  const json = await apiFetch<{ titles?: TitleDetail[] }>(`/api/title/trending?limit=${limit}`, { signal });
  return json.titles ?? [];
}
...
export async function getSimilarTitles(imdbId: string, limit = 6, signal?: AbortSignal): Promise<TitleDetail[]> {
  const id = encodeURIComponent(normId(imdbId));
  const json = await apiFetch<{ titles?: TitleDetail[] }>(`/api/title/${id}/similar?limit=${limit}`, { signal });
  return json.titles ?? [];
}
